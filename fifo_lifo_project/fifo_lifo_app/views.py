...
def request_things(request):
    if request.method == "POST":
        data = request.POST
        number_request = HelpRequest.objects.create(full_name_petitioner=request.POST['full_name_petitioner'])
        print(number_request)
        for number in range(int(data['amount_thing'])):
            RequestThings.objects.create(name_thing=data[f"name{number}"], number_request=number_request)
    return render(request, 'fifo_lifo_templates/home_page.html')
...
def donation_things(request):
    if request.method == "POST":
        data = request.POST
        number_donation = Donation.objects.create(full_name_donator=request.POST['full_name_donator'])
        for number in range(int(data['amount_thing'])):
            DonationThings.objects.create(name=data[f"name{number}"], number_donation=number_donation)
    return render(request, 'fifo_lifo_templates/home_page.html')
